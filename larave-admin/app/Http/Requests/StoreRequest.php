    ...
    public function rules()
    {
        ...
        switch (request()->getPathInfo()) {
            case '/menus':
                ...
                $rules['icon']  = 'required';
                $rules['uri']  = 'required|max:100';
                $rules['roles']  = 'required';
                break;
            case '/users':

                if($uid = request()->input('id')){
                    $rules['user_name'] = 'required|alpha_num|between:2,12|unique:admin_users,username,'.$uid;
                    $rules['pwd']  = 'nullable|alpha_num|between:6,12|confirmed';
                    $rules['email']  = 'required|email|unique:admin_users,email,'.$uid;

                }else{
                    $rules['user_name'] = 'required|alpha_num|between:2,12|unique:admin_users,username';
